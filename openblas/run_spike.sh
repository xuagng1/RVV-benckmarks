...
workload_dir=/home/radl/hdd/xg/workload/openblas/elf/openblas-benchmark-rv64gc-gcc12-o2
work_dir=/home/xugang/hdd/xg/test-log/spike/
core_name=openblas-1loop-rv64gc
...
run_name="$cur_date-$core_name$run_suffix"
...
run_ckpts=$(find $workload_dir -name '*.exe')
...
parallel -j $num_jobs --link --joblog $job_dir/parallel.log \
    ...
    $emu_bin --isa=rv64gcv_zba_zbb_zbc_zbs_zbkb_zbkc_zbkx_zknd_zkne_zknh_zksed_zksh_zicsr_zicntr \
    $pk -s {1} \
